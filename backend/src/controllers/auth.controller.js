...
export async function signup(req, res) {
  ...
  try {
    ...
    const newUser = await User.create({
      ...
      fullName,
      password,
      profilePic: normalizeAvatarUrl(randomAvatar, fullName),
    });
    ...
    try {
      await upsertStreamUser({
        ...
        image: normalizeAvatarUrl(newUser.profilePic, newUser._id.toString()),
      });
...
export async function login(req, res) {
  try {
    ...
    const normalizedProfilePic = normalizeAvatarUrl(
      user.profilePic,
      user._id?.toString() || user.fullName || "user"
    );

    if (user.profilePic !== normalizedProfilePic) {
      user.profilePic = normalizedProfilePic;
      await user.save();
    }
    ...
    try {
      await upsertStreamUser({
        ...
        name: user.fullName,
        image: normalizedProfilePic,
      });
    } catch (streamError) {
      console.log("Error syncing Stream user on login:", streamError.message);
    }
...
export async function onboard(req, res) {
  try {
    ...
    const updatedUser = await User.findByIdAndUpdate(
      ...
      {
        ...req.body,
        profilePic: normalizeAvatarUrl(req.body?.profilePic, userId.toString()),
        isOnboarded: true,
      },
      { new: true }
    );
